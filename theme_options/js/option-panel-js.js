...
/*Admin options pannal data value*/
	function commingsoon_option_data_save(id) 
	{ 	
		var commingsoon_lite_settings_save= "#commingsoon_lite_settings_save_"+id;
		var commingsoon_lite_theme_options = "#commingsoon_lite_theme_options_"+id;
		var commingsoon_lite_settings_save_success = commingsoon_lite_settings_save+"_success";
		var loding_image ="#commingsoon_loding_"+id;
		var commingsoon_loding_image = loding_image +"_image";
		
		jQuery(commingsoon_loding_image).show();
		jQuery(commingsoon_lite_settings_save).val("1");        
	    jQuery.ajax({
				   url:'admin.php?page=coming_soon',
				   type:'post',
				   data : jQuery(commingsoon_lite_theme_options).serialize(),
				   success : function(data)
					{  					
						jQuery(commingsoon_loding_image).fadeOut();						
						jQuery(commingsoon_lite_settings_save_success).show();
						jQuery(commingsoon_lite_settings_save_success).fadeOut(5000);
					}			
				});
	}
	
	/*Admin options value reset */
	function commingsoon_option_data_reset(id) 
	{   var r=confirm("Do you want reset your theme setting!")
		if (r==true)
		  {		var commingsoon_lite_settings_save= "#commingsoon_lite_settings_save_"+id;
				var commingsoon_lite_theme_options = "#commingsoon_lite_theme_options_"+id;
				var commingsoon_lite_settings_save_reset = commingsoon_lite_settings_save+"_reset";				
				jQuery(commingsoon_lite_settings_save).val("2");       
				jQuery.ajax({
						   url:'admin.php?page=coming_soon',
						   type:'post',
						   data : jQuery(commingsoon_lite_theme_options).serialize(),
						   success : function(data)
							{  	jQuery(commingsoon_lite_settings_save_reset).show();
								jQuery(commingsoon_lite_settings_save_reset).fadeOut(5000);
							}			
						});
		  }
		else
